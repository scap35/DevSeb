...
if ARGV.count < 1
  ...
  puts "#{__FILE__} <matrixRelease.xls> <logicalCatalog.xml>"
  exit
end
...
book.worksheet('Catalogue VHM').each do |row|
  ...
  if (ligne == 4)
	...
	entete = row
	 ...
	 ##unless (row[2] == 'Obsolete' || row[2] == 'N/A' || row[2].nil?)
